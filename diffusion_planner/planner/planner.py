...
class DiffusionPlanner(AbstractPlanner):
    # render=True 시 nuPlan 이 scenario 를 __init__ 에 주입 (비디오 파일명 log_name/token + GT overlay).
    ...
    def __init__(
        ...
        if render:
            ...
            self._video_dir = Path(save_dir) if save_dir else Path(os.getcwd())
    ...
    def initialize(self, initialization: PlannerInitialization) -> None:
        ...
        if self._ckpt_path is not None:
            ...
            # DDP-trained ckpt 는 "module." 접두사가 있어 벗겨낸다. 접두사가 하나도 없으면
            # (single-GPU/--ddp False 로 저장한 ckpt) 원본 dict 를 그대로 로드한다.
            model_state_dict = {k[len("module."):]: v for k, v in state_dict.items() if k.startswith("module.")}
            self._planner.load_state_dict(model_state_dict if model_state_dict else state_dict)
        else:
    ...
    def compute_planner_trajectory(self, current_input: PlannerInput) -> AbstractTrajectory:
        ...
        if self._render:
            try:
                self._render_frame(current_input, outputs)
            except Exception as e:
                warnings.warn(f"[render] frame skipped: {e}")

        return trajectory

    def _render_frame(self, current_input: PlannerInput, outputs: Dict[str, torch.Tensor]) -> None:
        """모델 출력(ego+neighbor, ego-centric local)을 render_from_simulation 으로 한 프레임."""
        pred = outputs['prediction'][0].detach().cpu().numpy()  # [P, T, 4] = xy+cos+sin (local)

        def to_xyh(a):  # [...,4] -> [...,3] (xy + heading)
            h = np.arctan2(a[..., 3], a[..., 2])[..., None]
            return np.concatenate([a[..., :2], h], axis=-1)

        ego_traj = to_xyh(pred[0])          # [T, 3]
        neigh = to_xyh(pred[1:]) if pred.shape[0] > 1 else None  # [N, T, 3]

        img = self._scene_render.render_from_simulation(
            current_input=current_input,
            initialization=self._initialization,
            route_roadblock_ids=self._route_roadblock_ids,
            scenario=self._scenario,
            iteration=current_input.iteration.index,
            planning_trajectory=ego_traj,
            predictions=neigh,
            return_img=True,
        )
    ...
    def generate_planner_report(self, clear_stats: bool = True):
        ...
        if self._render and self._imgs:
            ...
            if self._scenario is not None:
                name = f"{self._scenario.log_name}_{self._scenario.token}.mp4"
            else:
                name = "diffusion_render.mp4"
            out = self._video_dir / name
            imageio.mimsave(out, self._imgs, fps=10)
            print(f"\n[render] video saved: {out}\n", flush=True)
            self._imgs = []
        return report
