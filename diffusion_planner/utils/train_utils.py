...
class _LocalFileIO:
    """Minimal stdlib replacement for ``mmengine.fileio`` for local paths.

    The diffusion package only ever reads/writes local files here, so the
    remote-backend machinery of mmengine is unnecessary. This shim exposes the
    same three methods the call sites use (``get_text``/``get``/``put``) using
    plain Python I/O, avoiding the mmengine dependency in nuplan_py39.
    """
