...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_outputs, inputs):
        ...
        if self.training:
            ...
            # 평가 재현성: DIFFUSION_EVAL_SEED 가 설정되면 초기 노이즈 x_T 를 고정 시드로 생성
            # → 시뮬마다 동일 노이즈 → 결정론적 평가(체크포인트 간 차이=신호). 학습은 이 분기 미사용(무영향).
            _seed = os.environ.get("DIFFUSION_EVAL_SEED")
            if _seed is not None:
                _g = torch.Generator(device=current_states.device).manual_seed(int(_seed))
                _noise = torch.randn(B, P, self._future_len, 4, generator=_g, device=current_states.device)
            else:
                _noise = torch.randn(B, P, self._future_len, 4).to(current_states.device)
            xT = torch.cat([current_states[:, :, None], _noise * 0.5], dim=2).reshape(B, P, -1)
