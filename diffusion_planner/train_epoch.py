    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            ...
            [
                ...
                ), # 각도로 바꾸는 이유 => 불연속성 삭제 대칭 /주기성을 자연스럽게 인코딩
            ],
            dim=-1,
            )
            ...
            # LoRA: re-register in_proj_weight (demoted by loratorch.forward) so it
            # is present in state_dict() before ModelEma.update reads it.
            if getattr(args, 'use_lora_fine_tune', False):
                from diffusion_planner.model.lora.save import register_lora_weights
                register_lora_weights(model)
