...
class GPKGMapsDB(IMapsDB):
    ...
    def _load_map_data(self) -> None:
        ...
        # [PRACTICE-PATCH] MAP_LOCATIONS 4곳을 무조건 로드하던 것을, 실제로 로드 가능한 곳만 로드하도록 바꿨다.
        # 실습본 배포물에는 us-nv-las-vegas-strip 하나만 들어 있는데, NUPLAN_DATA_STORE 가 'local' 이면
        # BlobStore.save_to_disk 가 no-op 라서 나머지 3곳은 없는 파일을 읽다 예외로 죽는다.
        # 원격(S3) store 에서는 다운로드가 성공하므로 기존 동작이 그대로 유지된다.
        for location in MAP_LOCATIONS:
            try:
                self.load_vector_layer(location, DUMMY_LOAD_LAYER)
            except Exception as e:
                logger.warning(
                    "Map location '%s' is not available under map_root '%s' — skipping. (%s)",
                    location,
                    self._map_root,
                    e,
                )
